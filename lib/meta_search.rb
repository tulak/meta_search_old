module MetaSearch
  ...
  DEFAULT_WHERES = [
    ...
    ['contains', 'like', 'matches', {:types => (ALL_TYPES - BOOLEANS), :predicate => :matches, :formatter => '"%#{param}%"'}],
    ['does_not_contain', 'nlike', 'not_matches', {:types => (ALL_TYPES - BOOLEANS), :predicate => :does_not_match, :formatter => '"%#{param}%"'}],
    ['starts_with', 'sw', {:types => (ALL_TYPES - BOOLEANS), :predicate => :matches, :formatter => '"#{param}%"'}],
    ['does_not_start_with', 'dnsw', {:types => (ALL_TYPES - BOOLEANS), :predicate => :does_not_match, :formatter => '"%#{param}%"'}],
    ['ends_with', 'ew', {:types => (ALL_TYPES - BOOLEANS), :predicate => :matches, :formatter => '"%#{param}"'}],
    ['does_not_end_with', 'dnew', {:types => (ALL_TYPES - BOOLEANS), :predicate => :does_not_match, :formatter => '"%#{param}"'}],
    ['greater_than', 'gt', {:types => (NUMBERS + DATES + TIMES), :predicate => :gt}],
    ['less_than', 'lt', {:types => (NUMBERS + DATES + TIMES), :predicate => :lt}],
    ['greater_than_or_equal_to', 'gte', 'gteq', {:types => (ALL_TYPES - BOOLEANS), :predicate => :gteq}],
    ['less_than_or_equal_to', 'lte', 'lteq', {:types => (ALL_TYPES - BOOLEANS), :predicate => :lteq}],
